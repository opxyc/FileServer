<?php
  ...
  if(isset($_POST['projectName']) && $_POST['projectName']!="" && !(isset($_POST['loc']))){
    ...
    if(strpos($pn,'.')!==false || strpos($pn,'&')!==false || strpos($pn,'+')!==false)
      goto there;
    ...
    for($i=0;$i<count($directories);$i++){
      if(strpos($directories[$i],'&')!==false || strpos($directories[$i],'+')!==false){
        continue;
      }
      else{
    ...
    if($count==$i)
      ...
      header('Location:../../lis-dir.php?i='.$_POST['loc'].'&STATUS=Error --refer about.html#errorDir for details');
  }
  else{
    header('Location:../../');
  }
